...
export const SECTOR_TAGS = [
  ...
  'ev-charging', 'electric-vehicles', 'carbon-credits', 'carbon-capture-storage', 'direct-air-capture',
...
6. "raising_intent"
   SIGNALS (prospective / in-progress — the raise is OPEN, not closed):
   "in talks to raise", "seeking to raise", "looking to raise", "is raising",
   "plans to raise", "aims to raise", "hopes to raise", "expected to raise",
   "reportedly raising", "exploring a raise", "in the market for funding",
   "opened a round", "targeting a $X round", "kicks off a raise", "out raising".
   The SUBJECT is one named operating company (a startup/business) that is
   CURRENTLY seeking INVESTMENT capital (equity, venture, growth, or debt FROM
   INVESTORS) and has NOT yet closed the round.
   CRITICAL vs. "funding": if the round is DONE — past-tense "raised", "closed",
   "secured", "landed", "banked", "completed its round", or an amount already
   received — pick "funding", NOT raising_intent. Only pick raising_intent when the
   raise is ongoing / announced-but-not-closed. When in doubt and the money has
   clearly changed hands, choose "funding".
   DO NOT use raising_intent for: utility rate cases / "rate hikes", subsidies,
   payouts, grants, prizes/raffles, government tenders or auctions, procurement,
   project proposals, interconnection queues, insolvency/bankruptcy, or a fund/
   investor raising an LP fund (that's fund_close). Those are never raising_intent.

7. "funding"
   SIGNALS: "Series A/B/C/D/seed", "raised $XM", "closed Series X round", "strategic investment",
   "venture round", "growth round", "pre-seed" — a round that has CLOSED.
   The SUBJECT is an operating company (not a fund) that has raised capital.
   Only pick this if 1-6 above don't apply.
...
- "raising_intent" vs "funding" is a tense/status test, not a topic test: OPEN round →
  raising_intent, CLOSED round → funding. If both a past close and a new open raise appear,
  classify on the one the headline is about.
