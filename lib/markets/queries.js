/**
 * lib/markets/queries.js
 *
 * Shared data logic for the funding tracker so the admin routes and the
 * investor-dashboard routes return identical shapes from one source of truth.
 * Each function takes a Supabase client (service-role) and does no auth — the
 * calling route is responsible for gating (requireAdmin vs. signed-in investor).
 */
...
/** Visible funding events for the wire + charts (excludes hidden + market_stat aggregates). */
export async function getFundingEvents(supabase) {
...
/** Public-market basket quotes, ordered by market cap. */
export async function getQuotes(supabase) {
...
/** Forward-looking deal flow: currently raising, likely soon, recently raised. */
export async function getDealflow(supabase) {
  ...
  // ---- Likely raising soon (dormant): latest EQUITY round is 12-18 months old ----
  // Only equity rounds imply a follow-on is due — debt facilities, grants,
  // offtakes and project finance don't, so they're excluded from dormancy.
  const EQUITY_TYPES = new Set(["venture_equity", "corporate_strategic"]);
  const latestByCompany = new Map();
  for (const e of events) {
    if (!e.company_id) continue;
    if (!EQUITY_TYPES.has(e.type)) continue;
    if (!latestByCompany.has(e.company_id)) latestByCompany.set(e.company_id, e);
  }
  const dueIds = [];
  for (const [id, e] of latestByCompany) {
    const age = monthsAgo(e.announced_date);
    if (age >= 12 && age <= 18 && !["ipo", "public"].includes(e.stage)) dueIds.push({ id, last: e });
  }
  let likelyRaising = [];
  if (dueIds.length) {
  ...
  // Internal/test accounts and implausibly small raise targets are scrubbed so
  // the public panel stays credible.
  const INTERNAL_NAMES = new Set(["the energy pioneer", "ep network", "test company"]);
  const { data: raisingRaw } = await supabase.from("companies")
  ...
  const raising = (raisingRaw || [])
    .filter((c) => !INTERNAL_NAMES.has((c.name || "").trim().toLowerCase()))
    .map((c) => ({ ...c, raise_target: c.raise_target && c.raise_target >= 100000 ? c.raise_target : null }));
