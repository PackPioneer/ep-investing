...
const SECTORS = ["solar","wind_energy","battery_storage","green_hydrogen","nuclear_technologies","ev_charging","electric_vehicles","carbon_credits","carbon_capture_storage", "direct_air_capture","saf_efuels","electric_aviation","geothermal_energy","industrial_decarbonization","clean_cooking","grid_storage","grid_monitoring","agtech","maritime_shipping","fintech","information_services","energy_generation","energy_efficiency","energy_management"];
...
export default function InvestorDashboard() {
  ...
  const inputClass = "w-full text-sm px-3 py-2.5 rounded-lg border border-[#dbdfe4] bg-white focus:outline-none focus:border-[#2d6a4f]";
      ...
      <div className="flex flex-wrap gap-1.5">
        {options.map(o => (
          <button key={o} onClick={() => setActive(active === o ? null : o)}
            className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${active === o ? "bg-[#2d6a4f] text-white border-[#2d6a4f]" : "bg-white text-[#4a5568] border-[#dbdfe4] hover:border-[#2d6a4f]"}`}>
      ...
      <div className="flex-1 bg-[#f6f7f9] p-4 md:p-8 overflow-auto">
          ...
          <div className="flex flex-col gap-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={stat.label} className="bg-white border border-[#e8eaee] rounded-xl p-5">
              ...
              <div className="bg-white border border-[#e8eaee] rounded-2xl p-6">
                ...
                <div className="flex flex-col gap-2">
                  {companies.slice(0, 6).map(c => (
                    <div key={c.id} className="flex items-center justify-between py-1.5 border-b border-[#f6f7f9] last:border-0">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded bg-[#f2f4f6] flex items-center justify-center text-xs font-semibold text-[#2d6a4f] flex-shrink-0">
                          {c.name?.[0] || "?"}
                        </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={() => toggleSave(c.id)} className={`text-base ${isSaved(c.id) ? "text-[#2d6a4f]" : "text-[#dbdfe4]"}`}>
                          {isSaved(c.id) ? "★" : "☆"}
                        </button>
                      </div>
                    </div>
              ...
              <div className="bg-white border border-[#e8eaee] rounded-2xl p-6">
                ...
                <div className="flex flex-col gap-2">
                  {grants.slice(0, 5).map(g => (
                    <div key={g.id} className="py-1.5 border-b border-[#f6f7f9] last:border-0">
        ...
        {activeTab === "profile" && profileForm && (
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white border border-[#e8eaee] rounded-2xl p-6">
              <div className="flex items-center gap-5 mb-4">
                <div className="relative flex-shrink-0">
                  {investorLogoUrl ? (
                    <img src={investorLogoUrl} alt="Logo" className="w-16 h-16 rounded-full object-cover border border-[#e8eaee]" />
                  ) : (
                    <div className="w-16 h-16 rounded-full bg-[#f2f4f6] border border-[#e8eaee] flex items-center justify-center text-2xl font-semibold text-[#2d6a4f]">
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => setEditingProfile(v => !v)}
                    className="text-xs font-semibold border border-[#2d6a4f] text-[#2d6a4f] px-3 py-1.5 rounded-lg hover:bg-[#f2f4f6] transition-colors">
              ...
              {/* badges row */}
              <div className="flex flex-wrap gap-2 pt-3 border-t border-[#e8eaee]">
                {profile?.investor_type && (
                  <span className="text-[10px] px-2.5 py-1 rounded-full bg-[#f2f4f6] text-[#2d6a4f] border border-[#c8d8cc] font-medium">
                ...
                {profile?.location && (
                  <span className="text-[10px] px-2.5 py-1 rounded-full bg-white text-[#4a5568] border border-[#dbdfe4]">
                    {profile.location}
                  </span>
                )}
              </div>
            ...
            {editingProfile ? (
              /* EDIT MODE */
              <div className="bg-white border border-[#e8eaee] rounded-2xl p-6 flex flex-col gap-5">
                ...
                <div className="pt-4 border-t border-[#e8eaee]">
                  ...
                  {investorLogoUrl && (
                    <img src={investorLogoUrl} alt="Logo" className="w-14 h-14 rounded-full object-cover border border-[#e8eaee] mb-2" />
                  )}
                  <label className="cursor-pointer inline-flex items-center gap-2 border border-[#dbdfe4] text-sm text-[#4a5568] px-4 py-2 rounded-lg hover:border-[#2d6a4f] hover:text-[#2d6a4f] transition-all">
                ...
                <div className="pt-4 border-t border-[#e8eaee] flex items-center gap-3">
              ...
              <>
                ...
                {!profile?.focus && !profile?.thesis && !profile?.website && (
                  <div className="bg-[#f2f4f6] border border-dashed border-[#c8d8cc] rounded-2xl p-6 text-center">
            ...
            <div className="bg-white border border-[#e8eaee] rounded-2xl p-6">
              ...
              {grants.length > 0 ? (
                <div className="flex flex-col gap-3">
                  {grants.map(grant => (
                    <div key={grant.id} className="border border-[#e8eaee] rounded-xl p-4 hover:border-[#2d6a4f] transition-colors">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {grant.amount_max_usd && (
                              <span className="text-xs px-2 py-0.5 rounded-full bg-[#f2f4f6] text-[#2d6a4f] border border-[#c8d8cc]">
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-2">
                  {[
                    ...
                    <div key={col.key} className="bg-white border border-[#e8eaee] rounded-xl p-4" style={{ borderTop: `2px solid ${col.color}` }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  {[
                    ...
                    return (
                      <div key={col.key} className="bg-white border border-[#e8eaee] rounded-xl overflow-hidden" style={{ borderTop: `2px solid ${col.color}` }}>
                        <div className="flex items-center justify-between px-3 py-2 border-b border-[#e8eaee]">
                        ...
                        <div className="p-2 flex flex-col gap-2 min-h-[80px]">
                          {colCompanies.length === 0 ? (
                            ...
                            <div key={c.id} className="border border-[#e8eaee] rounded-lg p-2 bg-[#fafbfc]">
                              ...
                              <div className="flex items-center gap-1.5 flex-wrap mb-1">
                                {c.funding_stage && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#f2f4f6] text-[#4a5568] border border-[#dbdfe4]">{STAGE_LABELS[c.funding_stage] || c.funding_stage}</span>}
                                {(c.industry_tags || []).slice(0,1).map(t => <span key={t} className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#f2f4f6] text-[#2d6a4f] border border-[#c8d8cc]">{formatSector(t)}</span>)}
                              </div>
                                  ...
                                  <div className="flex gap-1 flex-wrap">
                                    {["watching","contacted","diligence","passed"].filter(s => s !== col.key).map(s => (
                                      <button key={s} onClick={() => setStage(c.id, s)}
                                        className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-[#e8eaee] bg-white text-[#4a5568] hover:border-[#2d6a4f] hover:text-[#2d6a4f]">
                                        {getStageLabel(s)}
                                      </button>
                                    ))}
        ...
        {activeTab === "feed" && (
          <>
            ...
            <div className="bg-white border border-[#e8eaee] rounded-2xl p-4 mb-4">
              <input type="text" placeholder="Search companies..."
                value={search} onChange={e => setSearch(e.target.value)}
                className="w-full text-sm px-3 py-2 rounded-lg border border-[#dbdfe4] bg-[#fafbfc] focus:outline-none focus:border-[#2d6a4f] mb-4" />
            ...
            <div className="flex flex-col gap-3">
              {filtered.length > 0 ? filtered.map(company => (
                <div key={company.id} className="bg-white border border-[#e8eaee] rounded-xl p-4 flex items-start justify-between hover:border-[#2d6a4f] transition-colors">
                  <div className="flex gap-3 items-start flex-1 min-w-0">
                    <div className="w-9 h-9 rounded-lg bg-[#f2f4f6] flex items-center justify-center text-sm font-semibold text-[#2d6a4f] flex-shrink-0 overflow-hidden">
                      {company.logo_url
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {company.funding_stage && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-[#f2f4f6] text-[#4a5568] border border-[#dbdfe4]">
                      ...
                      <div className="flex gap-1.5 flex-wrap">
                        ...
                        {(company.industry_tags || []).slice(0,2).map(t => (
                          <span key={t} className="text-xs px-2 py-0.5 rounded-full bg-[#f2f4f6] text-[#4a5568] border border-[#dbdfe4]">{formatSector(t)}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2 ml-3 flex-shrink-0">
                    <button onClick={() => toggleSave(company.id)}
                      className={`text-lg ${isSaved(company.id) ? "text-[#2d6a4f]" : "text-[#dbdfe4]"}`}>
                      {isSaved(company.id) ? "★" : "☆"}
                    </button>
