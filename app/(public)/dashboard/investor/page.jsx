      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="bg-white border border-[#e2e6ed] rounded-2xl p-7 mb-4">
          ...
          {companies.length > 0 ? (
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <Link href={"/companies/" + company.id}
                        className="text-sm font-semibold text-[#0f1a14] hover:text-[#2d6a4f]">
                        {company.name}
                      </Link>
                    ...
                    <div className="flex gap-2 mt-1.5">
                      {company.looking_to_raise && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">💰 Raising</span>
                      )}
                      {company.is_hiring && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-violet-50 text-violet-700 border border-violet-200">🙋 Hiring</span>
                      )}
                    </div>
                  </div>
                  <Link href={"/companies/" + company.id}
            ...
            <p className="text-sm text-[#718096]">No companies match your focus areas yet.</p>
          )}
        </div>
