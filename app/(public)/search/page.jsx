        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-14">
          ...
          <div className="hidden md:flex items-center gap-7 text-sm text-[#6b7a72]">
            {["Companies", "Investors", "Grants", "Categories", "Jobs", "Experts"].map(item => (
              <Link key={item} href={`/${item.toLowerCase()}`} className="hover:text-[#e8ede8] transition-colors">{item}</Link>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <Link href="/get-matched" className="text-sm text-[#6b7a72] border border-[#252c32] rounded-md px-3 py-1.5 hover:text-[#e8ede8] transition-all">Get matched</Link>
            <Link href="/get-matched" className="text-sm bg-[#c8f560] text-[#0a0d0f] font-semibold rounded-md px-4 py-1.5 hover:bg-[#d4ff6b] transition-all">Claim your company</Link>
          </div>
        </div>
      </nav>
