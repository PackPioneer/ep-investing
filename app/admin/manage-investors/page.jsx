...
export default function ManageInvestors() {
  ...
  const fetchData = useCallback(async (q) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/investors?q=${encodeURIComponent(q || "")}`);
      const data = await res.json();
      setRows(Array.isArray(data.investors) ? data.investors : []);
    } catch { toast.error("Failed to fetch investors"); }
    setLoading(false);
  }, []);
  ...
  const setHidden = async (id, name, hide) => {
    ...
    try {
      const res = await fetch("/api/admin/investors", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action: hide ? "hide" : "unhide" }) });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">Search the full investor directory. Hiding removes a VC from all public surfaces (reversible).</p>
      </div>
      ...
      <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 mb-6 focus-within:border-slate-400 transition-colors">
        ...
        <input placeholder="Search all investors by name or URL..." className="flex-1 bg-transparent text-sm outline-none text-slate-900 placeholder-slate-400" value={search} onChange={e => setSearch(e.target.value)} />
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="grid grid-cols-[1fr_auto] gap-4 px-5 py-3 border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wide">
          <span>Investor</span><span>Actions</span>
        </div>
        {rows.length === 0 && !loading && <div className="px-5 py-8 text-sm text-slate-400 text-center">No investors found.</div>}
        {rows.map(c => (
          <div key={c.id} className={`grid grid-cols-[1fr_auto] gap-4 items-center px-5 py-3 border-b border-slate-50 ${c.is_hidden ? "opacity-50" : ""}`}>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <a href={investorPath(c)} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-slate-900 hover:text-emerald-600 truncate">{c.name}</a>
                <span className="text-xs text-slate-300">#{c.id}</span>
                {c.type && <span className="text-[10px] font-mono text-slate-400 capitalize">{c.type.replace(/_/g, " ")}</span>}
