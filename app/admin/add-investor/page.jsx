...
export default function AdminAddInvestor() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/admin/add-investor", {
        method: "POST",
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="text-2xl font-semibold text-slate-900">Add investor</h1>
      </div>
      <p className="text-sm text-slate-500 mb-8">
        Paste a URL and firm name. The system will scrape a description and logo. Takes ~5 seconds. The new investor profile will be claimable — share the link with the prospect.
      </p>

      <div className="bg-white border border-slate-200 rounded-xl p-6 flex flex-col gap-5">
        <div>
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5 block">Firm name *</label>
          <input
            type="text"
            value={name}
        ...
        <div>
          ...
          <input
            ...
            placeholder="https://breakthroughenergy.org"
            disabled={submitting}
            className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-emerald-500 transition-colors disabled:bg-slate-50"
          />
        </div>
        ...
        <button
          ...
          {submitting ? "Creating… (this takes ~5s)" : <>Create investor <ArrowRight size={14} /></>}
        </button>
