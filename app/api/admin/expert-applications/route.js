    ...
    await getResend().emails.send({
      ...
      html: `
        <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; padding: 40px 24px; color: #0f1a14;">
          ...
          <p style="font-size: 15px; line-height: 1.7; color: #4a5568; margin-bottom: 32px;">We've sent a separate invitation email so you can set up your account. Once you've signed up, you'll be able to manage your profile and respond to enquiries.</p>
          <a href="${process.env.NEXT_PUBLIC_SITE_URL}/sign-up" style="display: inline-block; background: #2d6a4f; color: white; font-family: sans-serif; font-size: 14px; font-weight: 600; padding: 12px 24px; border-radius: 8px; text-decoration: none;">
            Set up your account →
          </a>
          <p style="font-size: 13px; color: #a0aec0; margin-top: 40px;">— The EP Investing Team</p>
        </div>
      `,
      text: `Hi ${expert.name}, your application to join the EP Investing expert network has been approved. We've sent a separate invitation email so you can set up your account at ${process.env.NEXT_PUBLIC_SITE_URL}/sign-up.`,
    });

    // Send Clerk invitation so expert can sign up and have their account linked.
    // The webhook (user.created) will set experts.clerk_user_id once they complete signup.
    try {
      const { clerkClient } = await import("@clerk/nextjs/server");
      const client = await clerkClient();
      await client.invitations.createInvitation({
        emailAddress: expert.email,
        redirectUrl: `${process.env.NEXT_PUBLIC_SITE_URL}/sign-up`,
        publicMetadata: { expert_id: expert.id },
        ignoreExisting: true,
      });
    } catch (inviteErr) {
      console.error("Clerk invitation failed (expert still approved):", inviteErr);
      // Non-fatal: approval succeeded; expert can sign up manually if needed.
    }
  }
