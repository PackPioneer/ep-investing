...
export async function POST(req) {
  ...
  const { name, email, firm, role, sectors, stages, check_sizes, geographies, thesis, how_heard } = body;
        ...
        <div style="background: white; border-radius: 16px; padding: 32px; border: 1px solid #e2e6ed;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ${[
              ...
              ["Stages", stages?.join(", ")],
              ["Check size", check_sizes?.join(", ")],
              ["Geographies", geographies?.join(", ")],
              ["How heard", how_heard],
            ].filter(([, v]) => v).map(([k, v]) => `
              <tr>
                <td style="padding: 8px 0; color: #718096; font-family: monospace; font-size: 11px; width: 110px; vertical-align: top;">${k}</td>
                <td style="padding: 8px 0; color: #0f1a14; font-size: 14px;">${v}</td>
              </tr>
            `).join("")}
          </table>
          ${thesis ? `<div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid #e2e6ed;"><p style="font-family: monospace; font-size: 11px; color: #718096; text-transform: uppercase; margin: 0 0 8px;">Thesis</p><p style="color: #4a5568; font-size: 13px; line-height: 1.6; margin: 0;">${thesis}</p></div>` : ""}
          <div style="margin-top: 24px;">
            <a href="${process.env.NEXT_PUBLIC_SITE_URL || "https://epinvesting.com"}/admin"
