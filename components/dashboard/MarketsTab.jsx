...
export default function MarketsTab({ isSaved = () => false, onToggleSave = () => {} }) {
  ...
  useEffect(() => {
    fetch("/api/dashboard/markets").then((r) => r.json()).then((d) => { setEvents(d.events || []); setMeta(d.meta || {}); setLoading(false); }).catch(() => setLoading(false));
    fetch("/api/dashboard/markets/quotes").then((r) => r.json()).then((d) => setQuotes(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);
  ...
  // Distinct companies in the current search — surfaced under the search bar with
  // a save star so tracking a company is one obvious tap.
  ...
  const stat = (l, v, s) => (
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      <p className="text-xs font-mono uppercase tracking-wider text-slate-400">{l}</p>
  ...
  return (
    <div>
      ...
      {quotes.filter((qq) => qq.price).length > 0 && (
        <div className="mb-5">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {quotes.filter((qq) => qq.price).map((qq) => (
              <div key={qq.ticker} className="flex-shrink-0 bg-white border border-slate-200 rounded-lg px-3 py-2 min-w-[110px]">
                <div className="text-xs font-semibold text-slate-800">{qq.ticker}</div>
                <div className="text-sm font-mono text-slate-900">${Number(qq.price).toFixed(2)}</div>
                <div className={`text-xs font-mono ${qq.change_pct >= 0 ? "text-emerald-600" : "text-red-500"}`}>{qq.change_pct >= 0 ? "+" : ""}{Number(qq.change_pct ?? 0).toFixed(1)}%</div>
              </div>
            ))}
          </div>
        </div>
                ...
                <tr key={e.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-3 py-2 text-slate-800 font-medium">
                    <span className="inline-flex items-center gap-1.5">
                      {e.company_id && (
                        <button onClick={() => onToggleSave(e.company_id)}
